...
module RailsAdmin
  module MainHelper
    ...
    # overwritting kaminari helper to prevent count (last_page?) call
    def link_to_next_page(scope, name, options = {}, &block)
      params = options.delete(:params) || {}
      param_name = options.delete(:param_name) || Kaminari.config.param_name
      last_page = scope.size < options[:page_items].to_i
      link_to_unless last_page, name, params.merge(param_name => scope.current_page + 1), options.reverse_merge(:rel => 'next') do
        block.call if block
      end
    end
  end
